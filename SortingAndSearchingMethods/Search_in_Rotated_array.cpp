...
int Search_Element_in_Rotated_Array(int n,int arr[],int target){
    ...
    while(start<=end){
        ...
        else if(arr[mid]>=arr[0]){// this means left side is sorted 
            // this if checks that target is in left side or not
        ...
        else { // this else shows (arr[mid]<=arr[0]) // right side sort hai
            //   
